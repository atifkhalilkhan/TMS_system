...
export default function ProjectScreen() {
    ...
    // dotted-circle single selection (independent)
    const [circleSelectedIds, setCircleSelectedIds] = useState<string[]>([]);
    ...
    const deleteTask = async (id: string) => {
        ...
        if (!error) {
            ...
            setCircleSelectedIds(prev => prev.filter(x => x !== id));
            message.success("Deleted");
        } else {
    ...
    // dotted-circle selection independent of checkboxes
    const toggleCircleSelection = (taskId: string) => {
        setCircleSelectedIds(prev => prev.includes(taskId) ? prev.filter(id => id !== taskId) : [...prev, taskId]);
    };
                ...
                <div className="group flex items-center border-b border-gray-100 py-3 hover:bg-gray-50/80 transition-all px-4">
                    <div className="flex items-center gap-3 flex-1 min-w-0" style={{ paddingLeft: `${level * 20}px` }}>
                        ...
                        <div className="flex items-center">
                            ...
                            {/* Dotted circle - independent: must never be affected by checkbox */}
                            <div
                                onClick={() => toggleCircleSelection(task.id)}
                                className="flex items-center justify-center cursor-pointer mr-2"
                                title={circleSelectedIds.includes(task.id) ? "Unselect" : "Select"}
                            >
                                {circleSelectedIds.includes(task.id) ? (
